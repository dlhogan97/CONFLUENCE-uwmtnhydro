#!/usr/bin/env python
"""
Download OpenET data for the Tuolumne River Basin

This script demonstrates how to use the OpenET utility to download
evapotranspiration timeseries data for the Tuolumne River lumped basin.

Usage:
    Interactive mode (prompts for options):
        python download_tuolumne_et.py
    
    Command-line mode:
        python download_tuolumne_et.py --option 1
        python download_tuolumne_et.py --option 2 --variable eto
        python download_tuolumne_et.py --option 3 --start-date 2019-01-01 --end-date 2022-12-31
...
# Configuration
SHAPEFILE_PATH = "/scratch/dlhogan/ess-project-data/domain_Tuolumne_River_lumped/shapefiles/catchment/Tuolumne_River_lumped_HRUs_GRUs.shp"
OUTPUT_DIR = "/scratch/dlhogan/ess-project-data/domain_Tuolumne_River_lumped/observations/et"
...
def download_ensemble_monthly(start_date: str, end_date: str, variable: str = 'et'):
    ...
    df = download_openet_data(
        ...
        interval='monthly',
        units='mm',
        output_filename=f'openet_{variable}_ensemble_tuolumne_monthly.csv'
    )
...
def download_all_models_monthly(start_date: str, end_date: str, variable: str = 'et'):
    ...
    df = client.get_multiple_models_from_shapefile(
        ...
        interval='monthly',
        units='mm',
        output_path=Path(OUTPUT_DIR) / f'openet_{variable}_all_models_tuolumne_monthly.csv'
    )
...
def download_ensemble_daily(start_date: str, end_date: str, variable: str = 'et'):
    ...
    df = download_openet_data(
        ...
        interval='daily',
        units='mm',
        output_filename=f'openet_{variable}_ensemble_tuolumne_daily.csv'
    )
...
def download_custom(start_date: str, end_date: str, variable: str, 
    ...
    if len(models) == 1:
        df = download_openet_data(
            ...
            interval=interval,
            units='mm',
            output_filename=f'openet_{variable}_{models[0]}_tuolumne_{interval}.csv'
        )
    else:
        client = OpenETClient(env_file=str(ENV_FILE))
        df = client.get_multiple_models_from_shapefile(
            ...
            interval=interval,
            units='mm',
            output_path=Path(OUTPUT_DIR) / f'openet_{variable}_{"_".join(models)}_tuolumne_{interval}.csv'
        )
...
def interactive_mode():
    ...
    print("=" * 60)
    print("OpenET Data Download for Tuolumne River Basin")
    print("=" * 60)
        ...
        if option == '1':
            ...
            if models is None:
                ...
                client.get_multiple_models_from_shapefile(
                    ...
                    interval=interval,
                    units='mm',
                    output_path=Path(OUTPUT_DIR) / f'openet_{variable}_all_models_tuolumne_{interval}.csv'
                )
            else:
...
def main():
    ...
    parser = argparse.ArgumentParser(
        description='Download OpenET data for Tuolumne River Basin',
...
Examples:
  python download_tuolumne_et.py                           # Interactive mode
  python download_tuolumne_et.py --option 1                # Ensemble monthly
  python download_tuolumne_et.py --option 2 --variable eto # All models, ref ET
  python download_tuolumne_et.py --option 4 --models ssebop,disalexi --interval daily
  python download_tuolumne_et.py --option 5 --id-column HRU_ID  # Per-polygon with HRU_ID
        """
    )
