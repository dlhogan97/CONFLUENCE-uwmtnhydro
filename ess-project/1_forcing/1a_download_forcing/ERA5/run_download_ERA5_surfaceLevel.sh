...
for y in $years; do
  ...
  for c in $coordinates; do
    for f in $forcing_path; do
      ...
      echo "Starting download: Year=$y, Months=$(printf '%02d' $year_start_month)-$(printf '%02d' $year_end_month), Coords=$c, Path=$f"
      
      python download_ERA5_surfaceLevel_annual.py "$y" "$c" "$f" "$year_start_month" "$year_end_month" &
      pid=$!
...
echo "Downloaded ERA5 surface level data for space (lat_max, lon_min, lat_min, lon_max) [${coordinates}] for time ${start_date} / ${end_date}" >> $log_path/$log_file # 2nd line, append to existing file